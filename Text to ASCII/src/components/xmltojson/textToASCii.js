...
const texttoascii = () => {
  ...
  const convertTextToASCII = () => {
    ...
    try {
      ...
      const asciiArray = Array.from(asciiInput).map(char => char.charCodeAt(0));
      const asciiString = asciiArray.join(' ');
      setTextOutput(asciiString);
      setError('');
    } catch (error) {
      setError('Error converting text to ASCII');
      setTextOutput('');
    }
};
      ...
      <div className="container">
        <div className="button-container">
          ...
          <input
            ...
            id="fileInput"
          />
          <button onClick={convertTextToASCII} className="button convert-button">
            Convert
          </button>
